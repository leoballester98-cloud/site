...
/* Grava o resultado da última execução. Como a resposta HTTP sai antes do
   trabalho terminar, ela não conta mais nada — sem isto, uma falha ficaria
   invisível pra quem olha o dashboard. */
async function anotar(estado: Record<string, unknown>) {
  try {
    const db = createClient(
      Deno.env.get('SUPABASE_URL')!,
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!,
    ...
    await db.from('config').upsert(
      { chave: 'robo_criativos', valor: { ...estado, quando: new Date().toISOString() },
        atualizado: new Date().toISOString() },
      { onConflict: 'chave' },
    );
  } catch (e) {
    console.error('não consegui anotar o estado:', e);
  }
}

/* Responde na hora e segue trabalhando.

   O cron chama isto via pg_net, cujo timeout o painel do Supabase limita a
   5000 ms — e esta função leva bem mais que isso, porque pagina anos de dados
   do Meta. O resultado eram 'Timeout of 1000 ms reached' a cada 5 minutos, com
   a chamada morta antes de gravar qualquer coisa.

   waitUntil mantém a tarefa viva depois da resposta, então o timeout do
   chamador deixa de importar: ele recebe o 202 em milissegundos. */
Deno.serve(() => {
  const trabalho = atualizar()
    .then(async (r) => { console.log('ok', r); await anotar({ ok: true, ...r }); })
    .catch(async (e) => { console.error('falhou', e); await anotar({ ok: false, erro: String(e) }); });

  // @ts-ignore EdgeRuntime é global do runtime do Supabase
  if (typeof EdgeRuntime !== 'undefined') EdgeRuntime.waitUntil(trabalho);

  return Response.json({ ok: true, estado: 'processando' }, { status: 202 });
});
